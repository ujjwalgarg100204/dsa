package codes.college.litcoder.module2.lab1;
...
public class SimpleTextEditor {
    ...
    public static void main(String[] args) {
        ...
        while (i < commands.length) {
            int increment = 2;
          switch (commands[i]) {
            case "1" -> editor.insert(commands[i + 1]);
            case "2" -> editor.delete(Integer.parseInt(commands[i + 1]));
...
class CustomStack {
    ...
    private final Deque<String> editor;
    private final Deque<String> temp;
    ...
    public void delete(int count) {
        while (true) {
          assert this.editor.peekLast() != null;
